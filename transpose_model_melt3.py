...
def read_csv_file(csv_file):
    ...
    data_chunks = pd.read_csv(csv_file, chunksize=10000)
    ...
    # Drop columns that don't meet the specified conditions
...
def process_data_with_pandas(header, data):
    ...
    print("All columns have been added based on the coding system, starting the pivot")

    # Optimize data types before the pivot_table
...
def optimize_data_types(data):
    # Convert numeric columns to smaller integer types
    numeric_columns = ['Numerator', 'Denominator', 'Rate']
    for column in numeric_columns:
        if data[column].max() <= 32767 and data[column].min() >= -32768:
            data[column] = data[column].astype('int16')
        else:
            data[column] = data[column].astype('int32')

    # Convert categorical columns to category type
    categorical_columns = ['CAMPUS', 'Previous Code', 'Student Group', 'Student Grade', 'Subject', 'Level',
                           'Student Group Prefix', 'Test', 'Type', 'Level/Standard', 'Year']
...
def main():
    ...
    try:
        ...
        data.to_csv('2018-assessment-performance-campus.csv', index=False)
